...
const requests = {
  ...
  post: (url, body, options = defaultHeaders) =>
    axios
      .post(url, options.formData ? buildFormData(body) : body, options)
      .then(responseBody),
  put: (url, body, options = defaultHeaders) =>
    axios
      .put(url, options.formData ? buildFormData(body) : body, options)
      .then(responseBody),
...
const Birthdays = {
  getBirthday: (guildId, userId) =>
    requests.get(`/birthday/${guildId}?user_id=${userId}`),
  updateBirthday: (guildId, body) => requests.put(`/birthday/${guildId}`, body),
  deleteBirthday: (guildId, userId) =>
    requests.delete(`/birthday/${guildId}?user_id=${userId}`),
  getBirthdaysConfig: (guildId) => requests.get(`/birthday-config/${guildId}`),
  updateBirthdayConfig: (guildId, body) =>
    requests.put(`/birthday-config/${guildId}`, body),
};
...
const Answers = {
  ...
  updateAnswerConfig: (guildId, body) =>
    requests.put(`/answer-config/${guildId}`, body),
};

const Holidays = {
  getHoliday: (guildId, date) =>
    requests.get(`/holiday/${guildId}?date=${date}`),
  ...
  updateHolidayConfig: (guildId, body) =>
    requests.put(`/holiday-config/${guildId}`, body),
};
...
const GuildSettings = {
  updateSettings: (guildId, body) =>
    requests.put(`/guild-settings/${guildId}`, body),
